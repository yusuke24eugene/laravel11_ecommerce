<x-layout :value="$search" :usertype="$usertype" :categories="$categories" :count="$count" :orders="$orders">
    <section style="background-color: #dcf0fa; border-radius: 10px;">
        <div class="text-center container px-4 py-5 text-center">
            @if ($countSearch === 0)
                <h4><strong>No results found on "{{ $search }}"</span></strong>
                </h4>
            @elseif ($count === 1)
                <h4><strong>{{ $countSearch }} result found on "{{ $search }}"</strong></h4>
            @else
                <h4><strong>{{ $countSearch }} results found on "{{ $search }}"</strong></h4>
            @endif
